...
class AddtlHF:
    def __init__(self, base_path):
        ...
        self.Rc_ephys = pd.read_hdf(find('*hf1_revchecker500ms*_ephys_props.h5',base_path)[0])
        # self.FmLt_ephys = pd.open_hdf(find('*fm1*_ephys_props.h5',base_path)[0])
        # self.Sn_world = xr.open_dataset(find('*_world.nc', base_path)[0])
        self.Rc_world = xr.open_dataset(find('*hf1_revchecker500ms*_world.nc', base_path)[0])
    ...
    def save(self):
        ...
        np.savez(self.savepath, rc=self.Rc_psth)#sn_on=self.on_Sn_psth, sn_off=self.off_Sn_psth, rf=self.rf_xy)#, rc=self.Rc_psth)
...
def main():
    ...
    #     '020522/J577RT'
    # ]
    recordings = ['/home/niell_lab/Mounts/Goeppert/nlab-nas/freely_moving_ephys/ephys_recordings/041422/J599RT/']
